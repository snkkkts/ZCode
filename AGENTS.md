...
- 开工前阅读 `docs/appearance/GOALS.md`、`docs/appearance/CHANGELOG.md` 和当前阶段 spec。
- 每次可交付变更必须同步更新变更记录：目标、修改文件、实现功能、解决的问题、真实验证结果、限制和下一步。
- 需求或设计改变时同步更新目标与 spec；明确区分计划、已实现、已验证。
- 并行任务明确文件或模块责任，不回退用户或其他 agent 的改动。
- 开发使用独立数据目录；不直接修改已安装的 ZCode 程序。
